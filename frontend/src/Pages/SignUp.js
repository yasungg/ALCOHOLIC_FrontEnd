...
const SignUp = () => {
    ...
    const termsChange = (e) => {
        ...
        if((textarea.scrollTop - (textarea.scrollHeight - textarea.offsetHeight)) > 0 && textarea.scrollTop > 0) {
            checkboxEnable(true);
    ...
    return(
        ...
        <hr className="hr"/>
        <div className="item">
            <textarea ref={textareaRef} name="terms" className="terms" cols="100" rows="10" readOnly>[회원가입 이용약관]
...
8. 회원이 이 약관에 동의함으로써 회사와의 이용계약이 성립됩니다. 회원은 이 약관에 따라 회사가 제공하는 서비스를 이용할 수 있습니다.
            ...
            <label className="termcheckbox">이용약관 확인 및 동의<input className="checkbox" type="checkbox" disabled={!ischeckBox}/>
            </label>           
        </div>
        <br />
        <div className="item">
